...
/* ANIMAL INFO HEADER */
.animal-header {
    background: linear-gradient(135deg, rgba(11, 94, 32, 0.05) 0%, rgba(66, 166, 91, 0.05) 100%);
    border: 1px solid rgba(11, 94, 32, 0.1);
    border-radius: 20px;
    padding: 25px;
    margin-bottom: 35px;
    text-align: center;
...
<div class="animal-header">
    ...
    <p><strong>ID:</strong> {{ $animal->id }} • <strong>Breed:</strong> {{ $animal->breed ?? 'Not specified' }}</p>
    <div class="status-badge status-{{ strtolower(str_replace(' ', '-', $animal->status)) }}">
...
<div class="form-header">
    ...
    <p style="color: #6b7280; margin: 0; font-size: 1.1rem;">Update animal information</p>
</div>

<div class="form-card">
    <form method="POST" action="{{ route('user.animals.update', $animal->id) }}" id="editAnimalForm">
        @csrf
        @method('PUT')
        ...
        <div class="form-grid">
            <div class="form-group">
                ...
                <input type="text" name="animal_type" value="{{ $animal->animal_type }}" required>
            </div>

            <div class="form-group">
                <label><i class="fas fa-paw"></i> Breed</label>
                <input type="text" name="breed" value="{{ $animal->breed }}" placeholder="e.g., Holstein, Angus">
            </div>
